trigger OpportunityTrigger on Opportunity (after undelete, before delete, after update, before Insert,after Insert) {
    ...
    {
        ASC_Opp_QuoteStartDate.oppRenewalCycleStartDate(trigger.new);
    }
    ...
    if(trigger.isafter)
    {
        if(trigger.isUpdate)        
        {
            OpportunityTriggerHandler.populateOpporunityNames(trigger.new, trigger.oldMap);    
